...
Ouch[![ouch](/final/ouch.jpg)](https://miro.medium.com/max/1600/1*ASPrAbd2qtFVSVuru7Y7Rw.png)
...
```{r}
m <- leaflet(libraries) %>%
  addProviderTiles(provider = "Ersi")   # Add default OpenStreetMap map tiles
...
There are a huge number free, beautiful "tiles" (customized map looks) that you can add to your interface - either as your base or on top of existing tiles.  You can see examples on this [equally beautiful GitHub page.](http://leaflet-extras.github.io/leaflet-providers/preview/index.html). I chose the `OpenStreetMap.HOT` interface because it felt a little more subtle and emphazied traffic markings less.
...
libmap <- leaflet() %>%
  ...
  addCircleMarkers(
    ...
    stroke = FALSE, fillOpacity = 0.5) %>%
  addCircleMarkers(
    lng = -112.0312464, lat = 33.38217769,popup = libraries$PlaceName,
    radius = 6,
    color = "red",
    stroke = FALSE, fillOpacity = 0.6
  )
